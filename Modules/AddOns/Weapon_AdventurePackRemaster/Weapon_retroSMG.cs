//sounds
datablock AudioProfile(retroSMGFireLoopSound)
{
   filename    = "./Sounds/Fire/New/MP7FireLoop.wav";
   description = BAADFireLoop3D;
   preload = true;
};

datablock DebrisData(retroSMGClipDebris)
{
	shapeFile = "./shapes/weapons/retroSMGMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(retroSMGClipExplosion)
{
	debris 					= retroSMGClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(retroSMGClipProjectile)
{
	explosion = retroSMGClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(retroSMGProjectile1 : gunProjectile)
{
   directDamage        = 15;//8;
...
datablock ProjectileData(retroSMGProjectile2 : retroSMGProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(retroSMGProjectile3 : retroSMGProjectile1)
{
...
datablock ItemData(retroSMGItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/retroSMG.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(retroSMGImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/retroSMG.dts";
   emap = true;
   ...
   doColorShift = false;
   colorShiftColor = retroSMGItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function retroSMGImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function retroSMGImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function retroSMGImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function retroSMGImage::onEmptyFire( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function retroSMGImage::onUntrig( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function retroSMGImage::onEmpty(%this,%obj,%slot)
{
...
function retroSMGImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "retroSMGProjectile1" TAB "retroSMGProjectile2" TAB "retroSMGProjectile3";
   ...
   if(!%obj.isPlayingLoopingAudio[%this,%slot+1])
   {
    	%obj.playAudio(%slot+1,retroSMGFireLoopSound);
...
function retroSMGProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function retroSMGProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function retroSMGProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function retroSMGImage::OnClipRemoved(%this, %obj, %slot)
{
          ...
          %p = new projectile()
          {
              datablock = "retroSMG" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0 0 -0.5"));
          };
          %p.explode();
}

function retroSMGImage::OnReloadB(%this, %obj, %slot) 
{
...
function retroSMGImage::OnReloadC(%this, %obj, %slot)
{
