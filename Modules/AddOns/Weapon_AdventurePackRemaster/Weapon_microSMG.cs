...
datablock AudioProfile(microSMGFireLoopSound)
{
   filename    = "./Sounds/Fire/New/Mac10FastFireLoop.wav";
   description = BAADFireLoop3D;
   preload = true;
};

datablock AudioProfile(microSMGFireSound)
{
   filename    = "./Sounds/Fire/microSMG.wav";
   description = AudioDefault3d;
   preload = true;
};

//muzzle flash effects
datablock ProjectileData(microSMGProjectile1 : gunProjectile)
{
   directDamage        = 17;//8;
...
datablock ProjectileData(microSMGProjectile2 : microSMGProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(microSMGProjectile3 : microSMGProjectile1)
{
   sound = BAADWhiz3Sound;
};

datablock DebrisData(microSMGClipDebris)
{
	shapeFile = "./shapes/weapons/microSMGMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(microSMGClipExplosion)
{
	debris 					= microSMGClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(microSMGClipProjectile)
{
	explosion = microSMGClipExplosion;
};
...
datablock ItemData(microSMGItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/microSMG.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(microSMGImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/microSMG.dts";
   emap = true;
   ...
   doColorShift = false;
   colorShiftColor = microSMGItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function microSMGImage::onMount( %this, %obj, %slot )
{
...
function microSMGImage::onUnMount( %this, %obj, %slot )
{
...
function microSMGImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function microSMGImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function microSMGImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function microSMGImage::onEmptyFire( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function microSMGImage::onUntrig( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function microSMGImage::onEmpty(%this,%obj,%slot)
{
...
function microSMGImage::onFire( %this, %obj, %slot )
{
   ...
   if(!%obj.isPlayingLoopingAudio[%this,%slot+1])
   {
    	%obj.playAudio(%slot+1,microSMGFireLoopSound);
...
function microSMGProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function microSMGProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function microSMGProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}


function microSMGImage::OnClipRemoved(%this, %obj, %slot)
{
...
function microSMGImage::OnReloadB(%this, %obj, %slot) 
{
...
function microSMGImage::OnReloadC(%this, %obj, %slot)
{
