//sounds
datablock AudioProfile(guerillaRifleFireLoopSound)
{
   filename    = "./Sounds/Fire/New/AK47FireLoop.wav";
   description = BAADFireLoop3D;
   preload = true;
};

datablock AudioProfile(guerillaRifleFireSound)
{
   filename    = "./Sounds/Fire/guerillaRifle.wav";
   description = AudioDefault3d;
   preload = true;
};

datablock DebrisData(guerillaRifleClipDebris)
{
	shapeFile = "./shapes/weapons/guerillaRifleMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(guerillaRifleClipExplosion)
{
	debris 					= guerillaRifleClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(guerillaRifleClipProjectile)
{
	explosion = guerillaRifleClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(guerillaRifleProjectile1 : gunProjectile)
{
   directDamage        = 25;//8;
...
datablock ProjectileData(guerillaRifleProjectile2 : guerillaRifleProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(guerillaRifleProjectile3 : guerillaRifleProjectile1)
{
...
datablock ItemData(guerillaRifleItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/guerillaRifle.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(guerillaRifleImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/guerillaRifle.dts";
   emap = true;
   ...
   // Projectile && Ammo.
   item = guerillaRifleItem;
   ammo = " ";
   projectile = guerillaRifleProjectile;
   ...
   doColorShift = false;
   colorShiftColor = guerillaRifleItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function guerillaRifleImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function guerillaRifleImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function guerillaRifleImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function guerillaRifleImage::onEmptyFire( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function guerillaRifleImage::onUntrig( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function guerillaRifleImage::onEmpty(%this,%obj,%slot)
{
...
function guerillaRifleImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "guerillaRifleProjectile1" TAB "guerillaRifleProjectile2" TAB "guerillaRifleProjectile3";
   ...
   if(!%obj.isPlayingLoopingAudio[%this,%slot+1])
   {
    	%obj.playAudio(%slot+1,guerillaRifleFireLoopSound);
...
function guerillaRifleProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function guerillaRifleProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function guerillaRifleProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function guerillaRifleImage::OnClipRemoved(%this, %obj, %slot)
{
          ...
          %p = new projectile()
          {
              datablock = "guerillaRifle" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0.4 0 -0.5"));
          };
          %p.explode();
}

function guerillaRifleImage::OnReloadB(%this, %obj, %slot) 
{
   %obj.schedule(0, "playThread", "3", "leftrecoil");
   schedule(0, 0, serverPlay3D, baadReload5Sound, %obj.getPosition());
   %obj.playThread(2,shiftUp);
}

function guerillaRifleImage::OnReloadC(%this, %obj, %slot)
{
   schedule(0, 0, serverPlay3D, baadReload15Sound, %obj.getPosition());
   schedule(150, 0, serverPlay3D, baadCock12Sound, %obj.getPosition());
