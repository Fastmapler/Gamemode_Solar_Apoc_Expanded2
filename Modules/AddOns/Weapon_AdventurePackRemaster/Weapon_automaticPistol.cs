//sounds
datablock AudioProfile(automaticPistolFireSound)
{
   filename    = "./Sounds/Fire/New/M9A3Fire.wav";
   description = AudioDefault3d;
   preload = true;
};

datablock DebrisData(automaticPistolClipDebris)
{
	shapeFile = "./shapes/weapons/automaticPistolMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(automaticPistolClipExplosion)
{
	debris 					= automaticPistolClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(automaticPistolClipProjectile)
{
	explosion = automaticPistolClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(automaticPistolProjectile1)
{
   directDamage        = 20;//8;
...
datablock ProjectileData(automaticPistolProjectile2 : automaticPistolProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(automaticPistolProjectile3 : automaticPistolProjectile1)
{
...
datablock ItemData(automaticPistolItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/automaticPistol.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(automaticPistolImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/automaticPistol.dts";
   emap = true;
   ...
   doColorShift = false;
   colorShiftColor = automaticPistolItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function automaticPistolImage::onMount( %this, %obj, %slot )
{
...
function automaticPistolImage::onUnMount( %this, %obj, %slot )
{parent::onUnMount(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot,-1);}

function automaticPistolImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function automaticPistolImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function automaticPistolImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function automaticPistolImage::onEmptyFire( %this, %obj, %slot )
{
   %obj.playThread(2, plant);
}

function automaticPistolImage::onEmpty(%this,%obj,%slot)
{
...
function automaticPistolImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "automaticPistolProjectile1" TAB "automaticPistolProjectile2" TAB "automaticPistolProjectile3";
   ...
   serverPlay3D( automaticPistolFireSound,%obj.getPosition());
...
function automaticPistolProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function automaticPistolProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function automaticPistolProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function automaticPistolImage::OnClipRemoved(%this, %obj, %slot)
{
          ...
          %p = new projectile()
          {
              datablock = "automaticPistol" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0 0 -0.5"));
          };
          %p.explode();
}

function automaticPistolImage::OnReloadB(%this, %obj, %slot) 
{
...
function automaticPistolImage::OnReloadC(%this, %obj, %slot)
{
