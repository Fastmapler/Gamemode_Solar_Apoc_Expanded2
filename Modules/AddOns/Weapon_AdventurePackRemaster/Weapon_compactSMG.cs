//sounds
datablock AudioProfile(compactSMGFireLoopSound)
{
   filename    = "./Sounds/Fire/New/MP5KFireLoop.wav";
   description = BAADFireLoop3D;
   preload = true;
};

datablock DebrisData(compactSMGClipDebris)
{
	shapeFile = "./shapes/weapons/compactSMGMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(compactSMGClipExplosion)
{
	debris 					= compactSMGClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(compactSMGClipProjectile)
{
	explosion = compactSMGClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(compactSMGProjectile1 : gunProjectile)
{
   directDamage        = 15;//8;
...
datablock ProjectileData(compactSMGProjectile2 : compactSMGProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(compactSMGProjectile3 : compactSMGProjectile1)
{
...
datablock ItemData(compactSMGItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/compactSMG.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
   ...
   //gui stuff
   uiName = "AP - Retro SMG"; //i know i know, should have renamed everything instead of of just the ui name, oh well
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(compactSMGImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/compactSMG.dts";
   emap = true;
   ...
   item = compactSMGItem;
   ammo = " ";
   projectile = compactSMGProjectile;
   ...
   doColorShift = false;
   colorShiftColor = compactSMGItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function compactSMGImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function compactSMGImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function compactSMGImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function compactSMGImage::onEmptyFire( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function compactSMGImage::onUntrig( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function compactSMGImage::onEmpty(%this,%obj,%slot)
{
...
function compactSMGImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "compactSMGProjectile1" TAB "compactSMGProjectile2" TAB "compactSMGProjectile3";
   ...
   if(!%obj.isPlayingLoopingAudio[%this,%slot+1])
   {
    	%obj.playAudio(%slot+1,compactSMGFireLoopSound);
...
function compactSMGProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function compactSMGProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function compactSMGProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function compactSMGImage::OnReloadA(%this, %obj, %slot) 
{
   %obj.playThread(2,plant);
   %obj.playThread(3,shiftRight);
   schedule(0, 0, serverPlay3D, baadReload15Sound, %obj.getPosition());  
   schedule(100, 0, serverPlay3D, baadReload8Sound, %obj.getPosition());  
}

function compactSMGImage::OnClipRemoved(%this, %obj, %slot)
{
          ...
          %p = new projectile()
          {
              datablock = "compactSMG" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0.3 0 -0.5"));
          };
          %p.explode();
}

function compactSMGImage::OnReloadC(%this, %obj, %slot) 
{
...
function compactSMGImage::OnReloadD(%this, %obj, %slot)
{
   %obj.schedule(0, "playThread", "2", "plant");
   %obj.schedule(50, "playThread", "3", "shiftLeft");
   schedule(0, 0, serverPlay3D, baadCock12Sound, %obj.getPosition());
}
