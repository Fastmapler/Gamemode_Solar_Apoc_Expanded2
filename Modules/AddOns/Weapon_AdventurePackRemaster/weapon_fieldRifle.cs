//sounds
datablock AudioProfile(fieldRifleFireSound)
{
   filename    = "./Sounds/Fire/fieldRifle.wav";
   description = AudioClose3d;
   preload = true;
};

datablock DebrisData(fieldRifleClipDebris)
{
	shapeFile = "./shapes/weapons/fieldRifleMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(fieldRifleClipExplosion)
{
	debris 					= fieldRifleClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(fieldRifleClipProjectile)
{
	explosion = fieldRifleClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(fieldRifleProjectile1 : gunProjectile)
{
   directDamage        = 30;//8;
...
datablock ProjectileData(fieldRifleProjectile2 : fieldRifleProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(fieldRifleProjectile3 : fieldRifleProjectile1)
{
...
datablock ItemData(fieldRifleItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/fieldRifle.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
////////////////
datablock ShapeBaseImageData(fieldRifleImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/fieldRifle.dts";
   emap = true;
   ...
   item = fieldRifleItem;
   ammo = " ";
   projectile = fieldRifleProjectile1;
   ...
   doColorShift = false;
   colorShiftColor = fieldRifleItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function fieldRifleImage::onSmoke( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
   %obj.playThread(2,plant);
}

function fieldRifleImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "fieldRifleProjectile1" TAB "fieldRifleProjectile2" TAB "fieldRifleProjectile3";
   ...
   serverPlay3d( fieldRifleFireSound, %obj.getPosition());
}

function fieldRifleProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function fieldRifleProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function fieldRifleProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function fieldRifleImage::OnReloadA(%this, %obj, %slot) 
{
...
function fieldRifleImage::OnReloadB(%this, %obj, %slot) 
{
