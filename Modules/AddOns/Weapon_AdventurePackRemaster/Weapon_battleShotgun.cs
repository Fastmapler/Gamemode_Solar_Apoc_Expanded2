...
//sounds
datablock AudioProfile(battleShotgunFireSound)
{
   filename    = "./Sounds/Fire/New/KS23Fire.wav";
   description = AudioDefault3d;
   preload = true;
};
datablock AudioProfile(battleShotgunCycleSound)
{
   filename    = "./Sounds/Reload/IthacaPump.wav";
   description = AudioClosest3d;
   preload = true;
};

//muzzle flash effects
datablock ProjectileData(battleShotgunProjectile : gunProjectile)
{
   directDamage        = 10;//8;
...
datablock ProjectileData(battleShotgunBlastProjectile : gunProjectile)
{
   directDamage        = 40;//8;
   explosion           = QuieterGunExplosion;
   impactImpulse       = 700;
...
datablock ItemData(battleShotgunItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/battleShotgun.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(battleShotgunImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/battleShotgun.dts";
   emap = true;
   ...
   // Projectile && Ammo.
   item = battleShotgunItem;
   ammo = " ";
   projectile = battleShotgunProjectile;
   ...
   doColorShift = false;
   colorShiftColor = battleShotgunItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function battleShotgunImage::onReloadSingle( %this, %obj, %slot )
{
...
function battleShotgunImage::onCockAmmo( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
...
function battleShotgunImage::onCock( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
   serverPlay3d( huntingShotgunCycleSound, %obj.getPosition() );
   %obj.playThread(3, "plant");
...
function battleShotgunImage::onEmptyFire( %this, %obj, %slot )
{
   %obj.playThread(2, plant);
}

function battleShotgunImage::onWait( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function battleShotgunImage::onEmpty(%this,%obj,%slot)
{
...
function battleShotgunImage::onFire( %this, %obj, %slot )
{
...
//	//%obj.setVelocity(VectorAdd(%obj.getVelocity(),VectorScale(%aimVec,"-2")));
	...
	%projectile = battleShotgunBlastProjectile;
	%spread = 0.0005;
	%shellcount = 1;
    ...
    %obj.schedule(0, playThread, 3, "shiftAway");
   serverPlay3d( battleShotgunFireSound, %obj.getPosition());
}

function battleShotgunProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function battleShotgunProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function battleShotgunProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
