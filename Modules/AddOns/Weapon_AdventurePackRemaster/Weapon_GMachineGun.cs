//sounds
datablock AudioProfile(GMachineGunFireLoopSound)
{
   filename    = "./Sounds/Fire/New/AK74UFireLoop.wav";
   description = BAADFireLoop3D;
   preload = true;
};

datablock AudioProfile(GMachineGunFireSound)
{
   filename    = "./Sounds/Fire/GMachineGun.wav";
   description = AudioDefault3d;
   preload = true;
};

datablock DebrisData(GMachineGunClipDebris)
{
	shapeFile = "./shapes/weapons/GMachineGunMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(GMachineGunClipExplosion)
{
	debris 					= GMachineGunClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(GMachineGunClipProjectile)
{
	explosion = GMachineGunClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(GMachineGunProjectile1 : gunProjectile)
{
   directDamage        = 22;//8;
...
datablock ProjectileData(GMachineGunProjectile2 : GMachineGunProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(GMachineGunProjectile3 : GMachineGunProjectile1)
{
...
datablock ItemData(GMachineGunItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/GMachineGun.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(GMachineGunImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/GMachineGun.dts";
   emap = true;
   ...
   // Projectile && Ammo.
   item = GMachineGunItem;
   ammo = " ";
   projectile = GMachineGunProjectile;
   ...
   doColorShift = false;
   colorShiftColor = GMachineGunItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function GMachineGunImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function GMachineGunImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function GMachineGunImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function GMachineGunImage::onEmptyFire( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function GMachineGunImage::onUntrig( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function GMachineGunImage::onEmpty(%this,%obj,%slot)
{
...
function GMachineGunImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "GMachineGunProjectile1" TAB "GMachineGunProjectile2" TAB "GMachineGunProjectile3";
   ...
   if(!%obj.isPlayingLoopingAudio[%this,%slot+1])
   {
    	%obj.playAudio(%slot+1,GMachineGunFireLoopSound);
...
function GMachineGunProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function GMachineGunProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function GMachineGunProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function GMachineGunImage::OnClipRemoved(%this, %obj, %slot)
{
          ...
          %p = new projectile()
          {
              datablock = "GMachineGun" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0.4 0 -0.5"));
          };
          %p.explode();
}

function GMachineGunImage::OnReloadB(%this, %obj, %slot) 
{
...
function GMachineGunImage::OnReloadC(%this, %obj, %slot)
{
