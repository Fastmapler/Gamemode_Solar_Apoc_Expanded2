//sounds
datablock AudioProfile(sniperRepeaterFireSound)
{
   filename    = "./Sounds/Fire/New/sniperRepeaterFire.wav";	
   description = AudioDefault3d;
   preload = true;
};

datablock DebrisData(sniperRepeaterClipDebris)
{
	shapeFile = "./shapes/weapons/sniperRepeaterMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(sniperRepeaterClipExplosion)
{
	debris 					= sniperRepeaterClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(sniperRepeaterClipProjectile)
{
	explosion = sniperRepeaterClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(sniperRepeaterProjectile1 : gunProjectile)
{
   directDamage        = 35;//8;
...
datablock ProjectileData(sniperRepeaterProjectile2 : sniperRepeaterProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(sniperRepeaterProjectile3 : sniperRepeaterProjectile1)
{
...
datablock ItemData(sniperRepeaterItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/sniperRepeater.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(sniperRepeaterImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/sniperRepeater.dts";
   emap = true;
   ...
   doColorShift = false;
   colorShiftColor = sniperRepeaterItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function sniperRepeaterImage::onUnMount( %this, %obj, %slot )
{
	...
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		...
		serverPlay3D(sniperRepeaterFireEndSound,%obj.getPosition());
	}
}

function sniperRepeaterImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function sniperRepeaterImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function sniperRepeaterImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function sniperRepeaterImage::onEmptyFire( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function sniperRepeaterImage::onUntrig( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function sniperRepeaterImage::onEmpty(%this,%obj,%slot)
{
...
function sniperRepeaterImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "sniperRepeaterProjectile1" TAB "sniperRepeaterProjectile2" TAB "sniperRepeaterProjectile3";
   ...
   serverPlay3d( sniperRepeaterFireSound, %obj.getPosition());
...
function sniperRepeaterProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function sniperRepeaterProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function sniperRepeaterProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function sniperRepeaterImage::OnClipRemoved(%this, %obj, %slot)
{
          ...
          %p = new projectile()
          {
              datablock = "sniperRepeater" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0.4 0 -0.5"));
          };
          %p.explode();
}

function sniperRepeaterImage::OnReloadB(%this, %obj, %slot) 
{
   schedule(0, 0, serverPlay3D, baadReload7Sound, %obj.getPosition());
   %obj.playThread(2,shiftUp);
}

function sniperRepeaterImage::OnReloadC(%this, %obj, %slot)
{
