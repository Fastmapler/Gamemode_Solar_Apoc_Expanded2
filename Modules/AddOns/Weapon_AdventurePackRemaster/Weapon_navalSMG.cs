//sounds
datablock AudioProfile(navalSMGFireLoopSound)
{
   filename    = "./Sounds/Fire/New/AUGA1FireLoop.wav";
   description = BAADFireLoop3D;
   preload = true;
};

datablock DebrisData(navalSMGClipDebris)
{
	shapeFile = "./shapes/weapons/navalSMGMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(navalSMGClipExplosion)
{
	debris 					= navalSMGClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(navalSMGClipProjectile)
{
	explosion = navalSMGClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(navalSMGProjectile1 : gunProjectile)
{
   directDamage        = 20;//8;
...
datablock ProjectileData(navalSMGProjectile2 : navalSMGProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(navalSMGProjectile3 : navalSMGProjectile1)
{
...
datablock ItemData(navalSMGItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/navalSMG.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(navalSMGImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/navalSMG.dts";
   emap = true;
   ...
   doColorShift = false;
   colorShiftColor = navalSMGItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function navalSMGImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function navalSMGImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function navalSMGImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function navalSMGImage::onEmptyFire( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function navalSMGImage::onUntrig( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function navalSMGImage::onEmpty(%this,%obj,%slot)
{
...
function navalSMGImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "navalSMGProjectile1" TAB "navalSMGProjectile2" TAB "navalSMGProjectile3";
   ...
   if(!%obj.isPlayingLoopingAudio[%this,%slot+1])
   {
    	%obj.playAudio(%slot+1,navalSMGFireLoopSound);
...
function navalSMGProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function navalSMGProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function navalSMGProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function navalSMGImage::OnClipRemoved(%this, %obj, %slot)
{
          ...
          %p = new projectile()
          {
              datablock = "navalSMG" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0.3 0 -0.5"));
          };
          %p.explode();
}

function navalSMGImage::OnReloadB(%this, %obj, %slot) 
{
...
function navalSMGImage::OnReloadC(%this, %obj, %slot)
{
