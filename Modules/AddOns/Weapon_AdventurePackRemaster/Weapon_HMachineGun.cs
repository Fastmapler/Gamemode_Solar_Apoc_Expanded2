//sounds
datablock AudioProfile(HMachineGunFireLoopSound)
{
   filename    = "./Sounds/Fire/New/M60E4FireLoop.wav";
   description = BAADFireLoop3D;
   preload = true;
};
datablock AudioProfile(HMachineGunFireEndSound)
{
   filename    = "./Sounds/Fire/New/HeavyLoopEnd.wav";
   description = BAADFire3D;
   preload = true;
};

datablock AudioProfile(HMachineGunFireSound)
{
   filename    = "./Sounds/Fire/newm60.wav";
   description = AudioDefault3d;
   preload = true;
};

//muzzle flash effects
datablock ProjectileData(HMachineGunProjectile1 : gunProjectile)
{
   directDamage        = 16;//8;
...
datablock ProjectileData(HMachineGunProjectile2 : HMachineGunProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(HMachineGunProjectile3 : HMachineGunProjectile1)
{
   sound = BAADWhiz3Sound;
};

datablock DebrisData(HMachineGunClipDebris)
{
	shapeFile = "./shapes/weapons/HMachineGunMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(HMachineGunClipExplosion)
{
	debris 					= HMachineGunClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(HMachineGunClipProjectile)
{
	explosion = HMachineGunClipExplosion;
};
...
datablock ItemData(HMachineGunItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/HMachineGun.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(HMachineGunImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/HMachineGun.dts";
   emap = true;
   ...
   doColorShift = false;
   colorShiftColor = HMachineGunItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function HMachineGunImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function HMachineGunImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function HMachineGunImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function HMachineGunImage::onUntrig( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function HMachineGunImage::onEmptyFire( %this, %obj, %slot )
{
	if(%obj.isPlayingLoopingAudio[%this,%slot+1])
	{
		%obj.isPlayingLoopingAudio[%this,%slot+1] = 0;
...
function HMachineGunImage::onEmpty(%this,%obj,%slot)
{
...
function HMachineGunImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "HMachineGunProjectile1" TAB "HMachineGunProjectile2" TAB "HMachineGunProjectile3";
   ...
   if(!%obj.isPlayingLoopingAudio[%this,%slot+1])
   {
    	%obj.playAudio(%slot+1,HMachineGunFireLoopSound);
...
function HMachineGunProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function HMachineGunProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function HMachineGunProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function HMachineGunImage::OnReloadA(%this, %obj, %slot) 
{
...
function HMachineGunImage::OnClipRemoved(%this, %obj, %slot)
{
   %obj.schedule(0, "playThread", "3", "plant");
          ...
          %p = new projectile()
          {
              datablock = "HMachineGun" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0.4 0 -0.5"));
          };
          %p.explode();
}

function HMachineGunImage::OnReloadC(%this, %obj, %slot)
{
...
function HMachineGunImage::OnReloadD(%this, %obj, %slot)
{
