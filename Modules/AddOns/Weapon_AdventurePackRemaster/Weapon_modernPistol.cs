//sounds
datablock AudioProfile(modernPistolFireSound)
{
   filename    = "./Sounds/Fire/New/USP45Fire.wav";
   description = AudioDefault3d;
   preload = true;
};

datablock DebrisData(modernPistolClipDebris)
{
	shapeFile = "./shapes/weapons/modernPistolMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(modernPistolClipExplosion)
{
	debris 					= modernPistolClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(modernPistolClipProjectile)
{
	explosion = modernPistolClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(modernPistolProjectile1 : gunProjectile)
{
   directDamage        = 15;//8;
...
datablock ProjectileData(modernPistolProjectile2 : modernPistolProjectile1)
{
   sound = BAADWhiz2Sound;
};

datablock ProjectileData(modernPistolProjectile3 : modernPistolProjectile1)
{
...
datablock ItemData(modernPistolItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/modernPistol.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(modernPistolImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/modernPistol.dts";
   emap = true;
   ...
   ammo = " ";
   projectile = modernPistolProjectile;
   ...
   doColorShift = false;
   colorShiftColor = modernPistolItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function modernPistolImage::onMount( %this, %obj, %slot )
{
...
function modernPistolImage::onUnMount( %this, %obj, %slot )
{parent::onUnMount(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot,-1);}

function modernPistolImage::onAmmoCheck( %this, %obj, %slot )
...
function modernPistolImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function modernPistolImage::onEmptyFire( %this, %obj, %slot )
{
   %obj.playThread(2, plant);
}

function modernPistolImage::onEmpty(%this,%obj,%slot)
{
...
function modernPistolImage::onFire( %this, %obj, %slot )
{
   ...
   hl2DisplayAmmo(%this,%obj,%slot);
    ...
    %projectiles = "modernPistolProjectile1" TAB "modernPistolProjectile2" TAB "modernPistolProjectile3";
   ...
   serverPlay3d( modernPistolFireSound, %obj.getPosition());
...
function modernPistolProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function modernPistolProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function modernPistolProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function modernPistolImage::OnClipRemoved(%this, %obj, %slot)
{
          ...
          %p = new projectile()
          {
              datablock = "modernPistol" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0 0 -0.5"));
          };
          %p.explode();
}

function modernPistolImage::OnReloadB(%this, %obj, %slot) 
{
...
function modernPistolImage::OnReloadC(%this, %obj, %slot)
{
