//sounds
datablock AudioProfile(servicePistolsilFireSound)
{
   filename    = "./Sounds/Fire/New/servicePistolsil.wav";
   description = AudioDefault3d;
   preload = true;  //I NEED A BETTER SOUND EFFECT SOISOISOISOISOISOISOI
};

//muzzle flash effects
datablock ProjectileData(servicePistolsilProjectile)
{
   directDamage        = 25;//8;
...
datablock ItemData(servicePistolsilItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/servicePistolSil.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(servicePistolsilImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/servicePistolSil.dts";
   emap = true;
   ...
   // Projectile && Ammo.
   item = servicePistolsilItem;
   ammo = " ";
   projectile = servicePistolsilProjectile;
   ...
   doColorShift = false;
   colorShiftColor = servicePistolsilItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function servicePistolsilImage::onMount( %this, %obj, %slot )
{
...
function servicePistolsilImage::onUnMount( %this, %obj, %slot )
{parent::onUnMount(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot,-1);}

function servicePistolsilImage::onAmmoCheck( %this, %obj, %slot )
{hl2AmmoCheck(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot);}

  /////// reload functions
function servicePistolsilImage::onReloadStart( %this, %obj, %slot )
{
   hl2DisplayAmmo(%this,%obj,%slot);
}

function servicePistolsilImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function servicePistolsilImage::onEmptyFire( %this, %obj, %slot )
{
   %obj.playThread(2, plant);
}

function servicePistolsilImage::onEmpty(%this,%obj,%slot)
{
...
function servicePistolsilImage::onFire( %this, %obj, %slot )
{
...
function servicePistolsilProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function servicePistolsilProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function servicePistolsilProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function servicePistolsilImage::OnClipRemoved(%this, %obj, %slot)
{
...
function servicePistolsilImage::OnReloadB(%this, %obj, %slot) 
{
...
function servicePistolsilImage::OnReloadC(%this, %obj, %slot)
{
