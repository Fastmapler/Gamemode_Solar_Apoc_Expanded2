//sounds
datablock AudioProfile(covertPistolFireSound)
{
   filename    = "./Sounds/Fire/New/M9SFire.wav";
   description = BAADFireSil3D;
   preload = true;
};

datablock DebrisData(covertPistolClipDebris)
{
	shapeFile = "./shapes/weapons/covertPistolMag.dts";
	lifetime = 2.0;
	minSpinSpeed = 700.0;
	maxSpinSpeed = 800.0;
	elasticity = 0.5;
...
datablock ExplosionData(covertPistolClipExplosion)
{
	debris 					= covertPistolClipDebris;
	debrisNum 				= 1;
	debrisNumVariance 		= 0;
	debrisPhiMin 			= 0;
	debrisPhiMax 			= 360;
...
datablock ProjectileData(covertPistolClipProjectile)
{
	explosion = covertPistolClipExplosion;
};

//muzzle flash effects
datablock ProjectileData(covertPistolProjectile : gunProjectile)
{
   directDamage        = 20;//8;
...
datablock ItemData(covertPistolItem)
{
   category = "Weapon";  // Mission editor category
   className = "Weapon"; // For inventory system

    // Basic Item Properties
   shapeFile = "./shapes/weapons/covertPistol.dts";
   rotate = false;
   mass = 1;
   density = 0.2;
   elasticity = 0.2;
...
//weapon image//
//////////////// 
datablock ShapeBaseImageData(covertPistolImage)
{
   // Basic Item properties
   shapeFile = "./shapes/weapons/covertPistol.dts";
   emap = true;
   ...
   ammo = " ";
   projectile = covertPistolProjectile;
   ...
   doColorShift = false;
   colorShiftColor = covertPistolItem.colorShiftColor;//"0.400 0.196 0 1.000";
...
function covertPistolImage::onMount( %this, %obj, %slot )
{
...
function covertPistolImage::onUnMount( %this, %obj, %slot )
{parent::onUnMount(%this,%obj,%slot); hl2DisplayAmmo(%this,%obj,%slot,-1);}

function covertPistolImage::onAmmoCheck( %this, %obj, %slot )
...
function covertPistolImage::onReload( %this, %obj, %slot )
{
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}

function covertPistolImage::onEmptyFire( %this, %obj, %slot )
{
   %obj.playThread(2, plant);
}

function covertPistolImage::onEmpty(%this,%obj,%slot)
{
...
function covertPistolImage::onFire( %this, %obj, %slot )
{
   ...
   serverPlay3D( covertPistolFireSound,%obj.getPosition());
...
function covertPistolProjectile1::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function covertPistolProjectile2::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}
function covertPistolProjectile3::damage( %this, %obj, %col, %fade, %pos, %normal ){   %damage = %this.directDamage;   %headshot = matchBodyArea( getHitbox( %obj, %col, %pos ), $headTest );   if ( %headshot )   {      %damage *= %this.headshotMultiplier;   }   %col.damage( %obj, %pos, %damage, %this.directDamageType );}

function covertPistolImage::OnClipRemoved(%this, %obj, %slot)
{
          ...
          %p = new projectile()
          {
              datablock = "covertPistol" @ (%obj.toolAmmo[%obj.currTool] <= 0 ? "Clip" : "") @ "Projectile";
              initialPosition = vectorAdd(%obj.getSlotTransform(0),vectorRelativeShift(%forward,%up,"0 0 -0.5"));
          };
          %p.explode();
}

function covertPistolImage::OnReloadB(%this, %obj, %slot) 
{
...
function covertPistolImage::OnReloadC(%this, %obj, %slot)
{
   schedule(0, 0, serverPlay3D, baadCock1Sound, %obj.getPosition());
   hl2AmmoOnReload(%this,%obj,%slot); 
   hl2DisplayAmmo(%this,%obj,%slot);
}
